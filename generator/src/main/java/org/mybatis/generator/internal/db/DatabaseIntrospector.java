...
public class DatabaseIntrospector {
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			for (IntrospectedColumn introspectedColumn : entry.getValue()) {
				...
				if (isTrue(tc.getProperty(PropertyRegistry.TABLE_USE_ACTUAL_COLUMN_NAMES))) {
					...
					sb.append('_');
					System.out.println("当前进程：DatabaseIntrospector.calculateExtraColumnInformation();else if");
					...
					System.out.println("当前进程：DatabaseIntrospector.calculateExtraColumnInformation();else ");
	...
	private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfiguration tc) throws SQLException {
		...
		while (rs.next()) {
			...
			// Statement stmt =
			// databaseMetaData.getConnection().createStatement();
			// ResultSet rset = stmt.executeQuery("show table status like '" +
			// atn.getTableName());
			// while (rset.next()) {
			//
			// }
