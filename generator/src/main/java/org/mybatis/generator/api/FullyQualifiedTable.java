...
public class FullyQualifiedTable {
	...
	public String getDomainObjectName() {
		if (stringHasValue(domainObjectName)) {
			return domainObjectName.startsWith("t") ? domainObjectName.substring(1) : domainObjectName;
		} else if (stringHasValue(runtimeTableName)) {
			System.out.println("当前进程：FullyQualifiedTable.getDomainObjectName();runtimeTableName=" + runtimeTableName);
			return getCamelCaseString(runtimeTableName, true);
		} else {
			System.out.println(
					"当前进程：FullyQualifiedTable.getDomainObjectName();introspectedTableName=" + introspectedTableName);
