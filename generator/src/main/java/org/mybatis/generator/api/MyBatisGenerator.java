...
public class MyBatisGenerator {
	...
	public void generate(ProgressCallback callback, Set<String> contextIds, Set<String> fullyQualifiedTableNames)
		...
		for (GeneratedXmlFile gxf : generatedXmlFiles) {
			...
			try {
				// File directory =
				// shellCallback.getDirectory(gxf.getTargetProject(),
				// gxf.getTargetPackage());
				// 修改Mapper.xml的生成路径。
				File mapDirec = new File(gxf.getTargetProject() + "/mappers/base");
				if (!mapDirec.exists()) {
					mapDirec.mkdir();
				}
				...
				if (targetFile.exists()) {
					if (gxf.isMergeable()) {
						...
						source = gxf.getFormattedContent();
						targetFile = getUniqueFileName(mapDirec, gxf.getFileName());
